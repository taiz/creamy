 ...
 * @param <T> 
 */
public class WizardBase<T> extends AvailableActivity {
    ...
    // 前へボタン
    @FXML private WizardBackButton prevButton;
    // 登録ボタン
    ...
    protected void setStatus(Status status) {
        if (status == Status.FIRST_WINDOW) {        
            // pathを書き換えて遷移先を指定する
            dialogForm.setPath("/WizardController/select");
            // Labelの状態を変更する
            ...
            // pathを書き換えて遷移先を指定する
            dialogForm.setPath("/WizardController/createComputer");
             // Labelの状態を変更する
            ...
            // pathを書き換えて遷移先を指定する
            dialogForm.setPath("/WizardController/confirm");
             // Labelの状態を変更する
            step1Label.setFont(Font.font("Arial", FontWeight.NORMAL, 14));
            step2Label.setFont(Font.font("Arial", FontWeight.NORMAL, 14));
