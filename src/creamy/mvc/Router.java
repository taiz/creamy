...
public class Router {
    ...
    public Response processRequest(Request request) {
        ...
        Object[] invokeParameter = getInvokeParameter(method, pathParams, request.getParams());
        Result result = invokeControllerMethod(controller, method, invokeParameter);
    ...
    private Response createResponse(Result result, String path, String methodName) {
        if (result.isRedirectResponse()) {
            ...
            return new Response(result.getStatus(), path, createActivity(methodName, result.getController()));
        } else if (result.isDataResponse()) {
            return new Response(result.getStatus(), path, result.getData());
        } else {
            return new Response(result.getStatus(), path);
        }
    }
    
    private Activity createActivity(String activityName, Controller controller) {
        ...
        return ActivityFactory.getInstance().createActivity(activityClass, controller);
    }
    ...
    */
    ...
    private Object[] getInvokeParameter(Method method, String[] pathParams, Map<String, Object> requestParams) {
        ...
        for (int i = 0; i < types.length; i++) {
            ...
            try {
                if ((bindAnnotation = getBindAnnotation(annotations[i])) != null) {
                    ...
                    invokeParams[i] = requestParams.get(bindAnnotation.key());
